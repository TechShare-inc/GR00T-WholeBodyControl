...
std::vector<uint8_t> BuildCommand(
  ...
  nlohmann::json header = {
      ...
      {"fields", nlohmann::json::array({
          ...
          {{"name", "playback_abort"}, {"dtype", "u8"}, {"shape", {1}}},
  ...
  message.insert(message.end(), {0, static_cast<uint8_t>(stop), 0,
                                 ...
                                 static_cast<uint8_t>(playback_abort)});
...
TEST(ZMQPlaybackProtocolTest, LoopbackMarkersDriveControllerReturnToStandProtocol) {
  ...
  ASSERT_TRUE(protocol.request_return_to_stand(
      completion->playback_id, completion->terminal_frame_index, *accepted, t0));
...
TEST(ZMQPlaybackProtocolTest, RejectedPlaybackStartDoesNotAdmitPoseFrames) {
  ...
  send(BuildCommand(true, false, false, 23));
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  manager.update();
  const auto playback_start = manager.ConsumePlaybackStart();
  ASSERT_TRUE(playback_start.has_value());
  ...
  EXPECT_FALSE(manager.GetLastAcceptedFrameIndex().has_value());
}

TEST(ZMQPlaybackProtocolTest, DecodesCorrelatedPlaybackAbort) {
  zmq::context_t context(1);
  zmq::socket_t publisher(context, zmq::socket_type::pub);
  publisher.bind("tcp://127.0.0.1:*");
  const auto endpoint = publisher.get(zmq::sockopt::last_endpoint);
  ...
  for (int i = 0; i < 3; ++i) {
    publisher.send(
        zmq::buffer(BuildCommand(false, false, false, 17, true)),
        zmq::send_flags::none);
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
